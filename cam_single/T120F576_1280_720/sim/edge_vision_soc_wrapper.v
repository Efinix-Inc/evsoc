...
////////////////////////////////////////////////////////////////////////////////
//
// NOTE: This wrapper bridges the legacy tb_soc.v testbench (and its port
// conventions) to a newer dual-camera-capable edge_vision_soc.v. The DUT's
// top-level interface changed significantly between project versions:
//   - master_rstn / offchip_rstn no longer exist on edge_vision_soc (reset
//     is now derived internally from PLL lock signals).
//   - i_pll_locked split into mipi_pll_locked + lvds_pll_locked.
//   - cam_rstn split into cam1_rstn + cam2_rstn (both now driven by DUT).
//   - mipi_inst1_* renamed to mipi_rx_inst1_* on the DUT, and a second
//     mipi_rx_inst2_* interface was added for camera 2.
//   - mipi_i2c_2_* (camera 2 I2C) added.
//   - DISPLAY_MODE parameter added.
//
// This wrapper is configured for SINGLE-CAMERA simulation only, matching
// what tb_soc.v actually drives. Camera 2's MIPI Rx inputs are tied off
// and its I2C bus is left floating.
//
///////////////////////////////////////////////////////////////////////////////////
...
module edge_vision_soc_wrapper #(
   ...
   //Display resolution setting passed through to edge_vision_soc.
   parameter DISPLAY_MODE          = "640x480_60Hz"
)(
   //Clock and Reset Pins
   input          master_rstn,      //Unused by DUT (kept for tb_soc.v port compatibility)
   input          i_pll_locked,     //Fed to both mipi_pll_locked and lvds_pll_locked on DUT
   ...
   input          soc_clk,       //50MHz
   input          dma_clk,       //80MHz
   input          offchip_rstn,     //Unused by DUT (kept for tb_soc.v port compatibility)
   output         cam_rstn,         //Mirrors DUT's cam1_rstn (camera 2's cam2_rstn is unused here)
   output         hdmi_rstn,
   //MIPI Control
...
//-----------------------------------------------------------------------------
// Camera 2 (unused, single-camera sim) - I2C bus left floating
//-----------------------------------------------------------------------------
wire     mipi_i2c_2_io_sda_writeEnable;
wire     mipi_i2c_2_io_sda_write;
wire     mipi_i2c_2_io_sda_read;
wire     mipi_i2c_2_io_scl_writeEnable;
wire     mipi_i2c_2_io_scl_write;
wire     mipi_i2c_2_io_scl_read;

assign   mipi_i2c_2_io_sda_read = 1'b1;  //Idle high, no device present
assign   mipi_i2c_2_io_scl_read = 1'b1;  //Idle high, no device present

//-----------------------------------------------------------------------------
// Camera 2 (unused, single-camera sim) - MIPI Rx tied off
//-----------------------------------------------------------------------------
wire         mipi_rx_inst2_DPHY_RSTN;
wire         mipi_rx_inst2_RSTN;
wire [3:0]   mipi_rx_inst2_VC_ENA;
wire [1:0]   mipi_rx_inst2_LANES;
wire         mipi_rx_inst2_CLEAR;

//-----------------------------------------------------------------------------
// Camera reset - cam2_rstn unused in single-camera sim
//-----------------------------------------------------------------------------
wire     cam2_rstn;
...
edge_vision_soc #(
   ...
   //Clock and Reset Pins
   .mipi_pll_locked                    (i_pll_locked),
   .lvds_pll_locked                    (i_pll_locked),
   ...
   .hdmi_rstn                          (hdmi_rstn),
   //MIPI Rx 1 Control
   .mipi_rx_inst1_DPHY_RSTN            (mipi_inst1_DPHY_RSTN),
   .mipi_rx_inst1_RSTN                 (mipi_inst1_RSTN),
   .mipi_rx_inst1_VC_ENA               (mipi_inst1_VC_ENA),
   .mipi_rx_inst1_LANES                (mipi_inst1_LANES),
   .mipi_rx_inst1_CLEAR                (mipi_inst1_CLEAR),
   //MIPI Rx 1 Video input
   .mipi_rx_inst1_HSYNC                (mipi_inst1_HSYNC),
   .mipi_rx_inst1_VSYNC                (mipi_inst1_VSYNC),
   .mipi_rx_inst1_CNT                  (mipi_inst1_CNT),
   .mipi_rx_inst1_VALID                (mipi_inst1_VALID),
   .mipi_rx_inst1_TYPE                 (mipi_inst1_TYPE),
   .mipi_rx_inst1_DATA                 (mipi_inst1_DATA),
   .mipi_rx_inst1_VC                   (mipi_inst1_VC),  
   .mipi_rx_inst1_ERR                  (mipi_inst1_ERR),
   //MIPI Rx 2 Control & Video input (unused, tied off)
   .mipi_rx_inst2_DPHY_RSTN            (mipi_rx_inst2_DPHY_RSTN),
   .mipi_rx_inst2_RSTN                 (mipi_rx_inst2_RSTN),
   .mipi_rx_inst2_VC_ENA               (mipi_rx_inst2_VC_ENA),
   .mipi_rx_inst2_LANES                (mipi_rx_inst2_LANES),
   .mipi_rx_inst2_CLEAR                (mipi_rx_inst2_CLEAR),
   .mipi_rx_inst2_HSYNC                (4'd0),
   .mipi_rx_inst2_VSYNC                (4'd0),
   .mipi_rx_inst2_CNT                  (4'd0),
   .mipi_rx_inst2_VALID                (1'b0),
   .mipi_rx_inst2_TYPE                 (6'd0),
   .mipi_rx_inst2_DATA                 (64'd0),
   .mipi_rx_inst2_VC                   (2'd0),
   .mipi_rx_inst2_ERR                  (18'd0),
`ifdef SIM
   //Simulation frame data from testbench
...
`endif
   ...
   //Camera 2 I2C (unused, tied off)
   .mipi_i2c_2_io_sda_writeEnable      (mipi_i2c_2_io_sda_writeEnable),
   .mipi_i2c_2_io_sda_write            (mipi_i2c_2_io_sda_write),
   .mipi_i2c_2_io_sda_read             (mipi_i2c_2_io_sda_read),
   .mipi_i2c_2_io_scl_writeEnable      (mipi_i2c_2_io_scl_writeEnable),
   .mipi_i2c_2_io_scl_write            (mipi_i2c_2_io_scl_write),
   .mipi_i2c_2_io_scl_read             (mipi_i2c_2_io_scl_read),
