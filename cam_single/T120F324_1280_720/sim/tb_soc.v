...
begin: main
   ...
   fork
      ...
      begin
         ...
         while(1) begin
            cam_valid_count = (DUT_wrapper.DUT.cam1_dma_wvalid) ? cam_valid_count + 1'b1 : cam_valid_count;
            @(posedge mipi_pclk);
         end
      end
      ...
      begin
         ...
         while (hw_accel_in_valid_count < (FRAME_WIDTH*FRAME_HEIGHT)) begin
            if (DUT_wrapper.DUT.hw_accel_dma1_rvalid && (&DUT_wrapper.DUT.hw_accel_dma1_rkeep) && DUT_wrapper.DUT.hw_accel_dma1_rready) begin
               $fwrite(outfile1, "%x\n", DUT_wrapper.DUT.hw_accel_dma1_rdata[31:0]);
